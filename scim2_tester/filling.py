...
def generate_random_value_for_type(
    conf: CheckConfig, obj: Resource, field_name: str
) -> tuple[Any, list[Resource]]:
    """Generate a random value for a given field name and object context."""
    garbages = []
    ...
    elif obj.__class__.model_fields[field_name].examples:
        value = random.choice(obj.__class__.model_fields[field_name].examples)
...
def fill_with_random_values(
    ...
    for field_name in field_names or obj.__class__.model_fields.keys():
        ...
        value, sub_garbages = generate_random_value_for_type(conf, obj, field_name)
        garbages += sub_garbages
